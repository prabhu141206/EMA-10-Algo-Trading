...
![Authentication Flow](assets/architecture/authentication_flow.png)
...
![Strategy Engine Flow](assets/architecture/Strategy_engine_flow.png)
...
![Virtual Trade Engine Flow](assets/architecture/virtual_trade_engine_flow.png)
...
The Database Logging System is responsible for asynchronously storing completed trade records in PostgreSQL. Instead of allowing the Virtual Trade Engine to perform direct database operations, completed trades are first placed into a queue. A dedicated Database Worker continuously processes the queue and writes trade information into the database. This asynchronous architecture ensures that real-time trade execution remains unaffected by database latency while maintaining a reliable trade history.

### Database Logging Flow

![Database Logging Flow](assets/architecture/db_flow.png)

### Components

**Trade Queue**
Acts as a temporary buffer between the Virtual Trade Engine and the database. It decouples trade execution from database operations, ensuring that real-time processing remains uninterrupted.

**Database Worker**
Continuously monitors the queue for completed trades and processes them asynchronously. This design improves scalability and prevents the trading engine from waiting on database operations.

**Trade Validation**
Performs basic validation before persisting trade information, ensuring that only complete and valid records are stored.

**PostgreSQL Storage**
Stores trade details such as entry price, exit price, stop loss, target, timestamps, trade status, and profit or loss for future analysis and reporting.

**Logging Completion**
Marks the successful completion of the logging process, allowing the worker to continue processing the next trade in the queue.
...
The Telegram Notification System provides real-time updates about the trading system by sending important trading events directly to the configured Telegram chat. It enables users to monitor the application's execution, trade lifecycle, and system status without accessing the application console.

### Notification Flow

![Telegram Notification Flow](assets/architecture/telegram_flow.png)

### Components

**System Event**
Represents an important event generated by the trading system that requires user notification.

**Message Builder**
Constructs a clear and structured message containing the relevant event information before sending it to Telegram.

**Telegram Bot API**
Sends the notification to the configured Telegram chat using the Telegram Bot API.

**Delivery Confirmation**
Receives the response from the Telegram API to verify that the notification was successfully delivered.

### Current Notifications

The system currently sends notifications for the following events:
...
The Graceful Shutdown module is responsible for safely terminating the trading system without losing critical trading information. Before shutting down, it ensures that all running components are stopped in the correct order, pending database operations are completed, active broker sessions are closed, and all allocated resources are released properly.

> **Note:** This module is currently under active development and its shutdown workflow may evolve as the project progresses.
